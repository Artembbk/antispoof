...
import antispoof.augmentations.spectrogram_augmentations
import antispoof.augmentations.wave_augmentations
from antispoof.augmentations.sequential import SequentialAugmentation
...
def from_configs(configs: ConfigParser):
    ...
    if "augmentations" in configs.config and "wave" in configs.config["augmentations"]:
        for aug_dict in configs.config["augmentations"]["wave"]:
            wave_augs.append(
                configs.init_obj(aug_dict, antispoof.augmentations.wave_augmentations)
            )
    ...
    if "augmentations" in configs.config and "spectrogram" in configs.config["augmentations"]:
        for aug_dict in configs.config["augmentations"]["spectrogram"]:
            spec_augs.append(
                configs.init_obj(aug_dict, antispoof.augmentations.spectrogram_augmentations)
            )
